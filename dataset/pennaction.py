...
class PennAction(torch.utils.data.Dataset):
    def __init__(self, cfg, pkl_name="train.pkl", mode="train", action="baseball_pitch"):
        self.cfg = cfg
        ...
        with open(os.path.join(self.cfg.data_loader.data_dir, pkl_name), 'rb') as f:
            self.dataset, self.action_to_indices = pickle.load(f)

        if action:
            indices = self.action_to_indices[PENN_ACTION_LIST.index(action)]
            self.dataset = [self.dataset[i] for i in indices]

        self.max_seq_len = max([d["seq_len"] for d in self.dataset])

        logger.info(f"Loaded PennAction dataset with {len(self.dataset)} samples")
